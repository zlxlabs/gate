...
or a JSON stdin envelope and performs no network access.
"""
...
def _verify_audit_identity(
    audit: dict[str, Any], *, repository_id: str, pr_number: int, head_sha: str,
    diff_digest: str, primary_run_id: str, primary_run_attempt: int,
) -> None:
    expected = {
        "repository_id": int(repository_id) if repository_id.isdigit() else repository_id,
        "pr": pr_number,
        "head_sha": head_sha,
    ...
    for field, expected_value in expected.items():
        ...
        if present is not None and audit[present] != expected_value:
            raise ValueError(f"audit {present} does not match requested binding")


def _receipt_fields(args: argparse.Namespace, envelope: dict[str, Any]) -> tuple[dict[str, Any], str, list[str]]:
    ...
    _verify_audit_identity(
        audit, repository_id=repository_id, pr_number=pr_number, head_sha=head_sha,
        diff_digest=diff_digest, primary_run_id=primary_run_id,
    ...
    evidence_refs = _value(args, envelope, "evidence_refs", "DISPOSITION_EVIDENCE_REFS")
    if isinstance(evidence_refs, str):
        evidence_refs = [item for item in evidence_refs.splitlines() if item]
    if not isinstance(evidence_refs, list) or not evidence_refs or any(not isinstance(item, str) or not item.strip() for item in evidence_refs):
        raise ValueError("at least one immutable evidence ref is required")
    evidence_refs = list(dict.fromkeys(evidence_refs))
    evidence_manifest_digest = _sha256_json(evidence_refs)
    ...
    return fields, reason, evidence_refs
...
def issue(args: argparse.Namespace, envelope: dict[str, Any]) -> int:
    fields, reason, evidence_refs = _receipt_fields(args, envelope)
    ...
    for command in ("issue", "revoke"):
        ...
        if command == "issue":
            ...
            for name in (
                ...
                "audit-digest", "evidence-manifest-digest", "disposition",
            ):
