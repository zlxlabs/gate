...
> 状态：当前 clean-streak 实现契约以 `docs/sessions/260925-disposition-exit/design.md` 为准；2026-09-15 的 record-only 决策背景保留于 `docs/sessions/260915-disposition-record-only/design.md`，并已由 2026-09-25 owner 裁决重开。
> 跨 run 的 `gate-disposition-receipt-v3-*` 仍由 GitHub ∪ Silo 双读；disposition workflow 写 Silo `d30/`，audit 读取路径不变。
>
...
且 verdict ∈ {pass, fail}
且当前 P1 finding 集合在扣除所有有效、active、精确绑定的 disposition 回执后为空
```

`unavailable`、`not_expected`、reviewer 自报 `waived` 都不是 clean。v2 没有标签豁免，唯一受控出口是 `gate-disposition`：有证据且绑定当前轮的回执逐条移除其指向的 P1；全部覆盖后本轮按无 P1 计入 clean streak，部分覆盖仍由剩余 P1 阻断。缺失、失效或不可读取的回执只影响回执诊断，不改变原本 gate 判定；canonical primary / audit 自身仍按既有规则 fail-closed。
...
| ledger 目前把 PR 评论中的 `Codex finding disposition` 解析进 `finding_dispositions` | `.github/actions/review-ledger/build_ledger.py:73-85,470-496` | 旧评论格式只作观察数据；只有 aggregator 消费的有效 `gate-disposition` 回执可以解除其精确指向的 P1。 |
...
- `audit_digest`：`canonical_audit_digest` 对 canonical scope、排序后的 findings 和 verdict 生成的 SHA-256；忽略 duration/tokens/timestamps 等 runtime 噪声。receipt 与 disposition 都绑定此 digest；不得用 findings 子集 digest 代替。
...
派生 state 最小字段为：`schema_version`、`epoch`、`clean_streak`、`eligible_rounds`、`unavailable_streak`、已消费 `processing_key`/`round_key` 集合的 digest、当前 terminal decision。逐条 finding 只存在于当轮 evidence receipt，不进入跨轮 lineage；gate-disposition receipt 只以当前 epoch + audit digest + exact finding id/key 消费。
...
状态简写：`C` = collecting（`eligible_rounds < max_rounds` 且 `clean_streak < N`），`U` = collecting 但有不可用预算，`T` = converged/terminal replay，`M` = `manual_required`，`F` = fail-closed/state lost。当前 P1 集合先按有效的逐条 disposition 回执扣减，再进入本状态机；`gate-disposition` 不改写 canonical audit，也不恢复已终止或损坏的历史 state。回执契约见 `docs/sessions/260925-disposition-exit/design.md`。

| 状态 \ 事件 | 新 major / blocker | 无新 finding | disposition 全覆盖 | disposition 拒绝/部分覆盖 | rerun | 新 commit 改 digest |
|---|---|---|---|---|---|---|
| `C` | `eligible+1, streak=0 → C/M`（到 cap 进 M）；`test_nonempty_p1_resets_streak_even_when_finding_ids_repeat` | `eligible+1, streak+1 → T/C/M`（`streak=N` 先 T）；`test_clean_threshold_wins_over_max_rounds_on_same_event` | 当前 digest 的全部 P1 被合法回执覆盖时按“无 P1”，部分覆盖仍按剩余 P1；`test_partial_disposition_stays_blocked` | 拒绝的回执只留诊断，未覆盖 P1 继续阻断；`test_rejected_disposition_cannot_advance_streak` | 同 `processing_key` 或 `round_key` → no-op；新 digest 按本行新 major/clean 重入；`test_rerun_same_audit_is_not_a_second_round` | 先丢弃旧 epoch，再把当前 audit 作为新 epoch 首轮：clean 则 `streak=1`，P1 则 `streak=0`；`test_scope_digest_change_starts_zero_generation` |
| `U` | 有效 primary 中断 unavailable streak，按 `C` 的新 major；`test_eligible_round_resets_unavailable_budget` | 中断 unavailable streak，按 `C` 的无 finding；`test_clean_round_resets_unavailable_budget` | 全部 P1 覆盖按 clean，否则按剩余 P1；`test_waiver_and_unavailable_counters_are_independent` | 无效回执不修复 P1；剩余 P1 继续 blocked；`test_rejected_disposition_cannot_advance_streak` | 相同 key no-op；新的 audit 先清掉 unavailable streak 再按内容归类；`test_duplicate_unavailable_receipt_is_idempotent` | 新 epoch 的 unavailable streak 和 eligible/streak 全为零，再评估当前首轮；`test_new_epoch_drops_unavailable_history` |
| `T` | 不消费旧预算，当前 head 出现未覆盖 active P1 → M；`test_terminal_replay_with_new_finding_requires_manual` | 保持 T，输出 `terminal_replay`，不加计数；`test_terminal_replay_does_not_consume_round` | 当前 digest 全覆盖 → T；部分覆盖仍 M；`test_terminal_replay_consumes_only_matching_disposition` | 拒绝仅记诊断，不改变 T；`test_terminal_replay_rejects_invalid_disposition` | 同 digest replay 保持 T；新 digest 在同 epoch 不是“继续 streak”，而是 M，等待新 generation；`test_converged_state_cannot_be_extended_by_rerun` | 生成新 epoch，清零后按首轮重新开始；`test_converged_state_resets_on_head_change` |
...
### 2.3 轴 C：逐条 gate-disposition 回执生命周期

回执接受的操作者身份不作为人工审批证明；owner 接受身份不可证，出口由反证/跟踪 issue、精确绑定和 terminal/ledger 留痕约束。
...
| disposition receipt artifact | v3 canonical bytes：两种处置之一、当前绑定、false-positive 反证或 deferred issue；跨 run 双读 GitHub Actions artifact 与 Silo `d30/<repo_id>/gate-disposition-receipt-v3-*` | 无证据放行、标签豁免、全局忽略规则、可变“active=true”旗标 | reducer 校验并仅扣减精确匹配的当前 P1；旧 v1/v2 receipt 不解除阻断；`tests/test_gate_convergence_artifact.py::test_disposition_producer_writes_minimal_receipt_bytes_from_raw_audit` |
...
绝不放进 PR 评论的状态清单：`clean_streak`、`eligible_rounds`、`unavailable_streak`、`last_run_id`、`last_run_attempt`、`epoch` 的唯一游标、`state_hash`、`round_key` 去重表、任何“当前有效 waiver”布尔值，以及用于 PATCH/If-Match 的版本号。评论可以展示这些值的**本 run 派生摘要**，但机器不能读回它们作判定；正式回执仍是 immutable artifact，不接受评论或 `codex-review-waived` 标签。
...
- 只把当轮 P1 evidence 放入 convergence receipt；不保存前轮 finding 集合、lineage、line、文本 fingerprint。当前 aggregator 同时消费 v3 gate-disposition receipt，仅将有效覆盖的具体 finding 从本轮 P1 集合扣除。
...
### 增量 2：当前轮逐条 gate-disposition 受控出口
...
1. 增量 1 未通过轴 A 全格矩阵，禁止接 disposition workflow。
2. disposition 绑定、证据、tier 或 receipt 留痕测试未通过时，禁止解除 P1；Required Check 按当前有效回执覆盖后的 P1 集合判定，不接受 reviewer 自报 waived。
...
4. canary 后发现无效 receipt 影响 P1、clean streak 或 Required Check，立即关闭 convergence consumer/feature pin；不得用空 commit、label、admin bypass 或手改评论补洞。

本设计的收敛判据仍是：**只对当前 epoch 的 canonical primary 和有效 active disposition 回执计算本轮剩余 P1；每张回执只覆盖一个精确 finding，所有 P1 均覆盖后本轮按 clean 样本进入 streak；无效回执不扣减 P1。跨轮只重放 convergence immutable artifact，不把 GitHub 评论或标签当可变状态。**
